...
- 💡 逻辑: [用 2-3 句话深度分析：①先说明当前时间距预计最热时段还有多久；②如果实测温度已经超过了 DEB 融合值和预报值，说明预报集体低估了——此时不要再引用这些被打脸的预报值来论证"难以升温"，而应该分析当前风速云量等动力条件是否支持继续冲击下一个 WU 整数；③如果实测还没到预报值，分析能否在剩余时间追上。请使用具体数值。]
- 🎯 置信度: [1-10]/10
"""
