...
5. **概率估算**：综合 DEB 融合预测值、集合预报区间、多模型分歧度、当前实测趋势，对最可能的 2-4 个 WU 结算整数温度给出概率估算（必须加起来 = 100%）。DEB 融合值应作为概率分布的中心锚点。

【输出要求】
1. **禁止废话**，整体控制在 150 字以内。
2. 严格按照以下 HTML 格式输出:

🤖 <b>Groq AI 决策</b>
- 🎲 盘口: [必须明确指出最热时段（如：预计最热在 14:00-16:00）以及当前的博弈区间（如：锁定在 27°C 或 28°C 之间博弈）。若已明确降温，请直接给出死盘结论（如：已过最热点且降温，锁定在 X 度结算，悬念终止）。]
- 💡 逻辑: [不要重复模版例子！请使用一句话提炼机场实测(如风速风向、云量、气温变化趋势)及热力动力因子。]
- 📊 概率: [列出最可能的 2-4 个结算温度及概率。格式示例：3°C(15%) | 4°C(55%) | 5°C(25%) | 6°C(5%)]
- 🎯 置信度: [1-10]/10
"""
